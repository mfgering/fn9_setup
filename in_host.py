...
def cmd_add_storage(args):
    ...
    for mountpoint in get_mountpoints(fn_host):
        if mountpoint['source'] == source:
            raise ValueError("That source already exists in jail %s." % mountpoint['jail'])
...
def cmd_add_user(args):
    (fn_host, username, full_name, password, uid, sudo) = args[0:6]
...
if __name__ == '__main__':
    commands = ['test', 'create_jail', 'add_storage', 'add_user', 'update_ssh_key']
    if len(sys.argv) > 1:
        cmd_name = sys.argv[1]
